...
public interface IInsert<TInsert, T, T2, T3> : ISqlStatement where TInsert : class where T : class where T2 : class where T3 : class
{
    /// <summary>
    ...
    /// check that will short-circuit the statement if the result of <see cref="IInsert{TInsert, T, T2, T3}"/> did not affect any rows.
    /// </summary>
    /// <remarks>
    /// It is recommended that building of the next statement be done using the fluent build methods on this object.  This helps to 
    /// ensure the last statement in the chain is able to compile all preceding statements into one. If you choose to break up 
    ...
    /// Method will use the of the public properties returned from the <typeparamref name="TProjection"/>'s GetProperties()
    /// method as the insert ordinal position.
    /// </summary>
    /// <typeparam name="TProjection">Projection model type.</typeparam>
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> Into<TProjection>() where TProjection : class;
    /// <summary>
    /// Sets the target columns to use in the INTO clause of the statement.
    /// </summary>
    /// <param name="columns">INTO target columns.</param>
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> Into(Expression<Func<TInsert, object>> columns);
    /// <summary>
    ...
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> OutputTo<TOutputTo>(Expression<Func<TInsert, TOutputTo>> statement) where TOutputTo : class;
    /// <summary>
    ...
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> Select(Expression<Func<T, T2, T3, object>> columns);
    /// <summary>
    /// Join tables <typeparamref name="TTable1"/> and <typeparamref name="TTable2"/> using Primary Key field identified
    /// in <see cref="TableNameAttribute"/>. By convention, method will assume Primary Key name in
    ...
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> Join<TTable1, TTable2>(JoinType joinType = JoinType.Inner) where TTable1 : class where TTable2 : class;
    /// <summary>
    ...
    /// </remarks>
    IInsert<TInsert, T, T2, T3> Join(Action<IJoinMapper> mapper);
    /// <summary>
    /// Create join condition between two table entities.  Method can be called multiple times per statement.
    /// </summary>
    ...
    /// <param name="joinType">Join type.</param>
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> Join(Expression<Func<T, T2, T3, bool>> columns, JoinType joinType);
    /// <summary>
    ...
    /// one or two key pair(s).</param>
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> JoinInner(Expression<Func<T, T2, T3, bool>> columns);
    /// <summary>
    ...
    /// one or two key pair(s).</param>
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> JoinLeftOuter(Expression<Func<T, T2, T3, bool>> columns);
    /// <summary>
    ...
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain query methods.</returns>
    IInsert<TInsert, T, T2, T3> Where(Expression<Func<T, T2, T3, bool>> expression);
    /// <summary>
    ...
    IInsert<TInsert, T, T2, T3> WhereExists<TExists, TOuter>(Expression<Func<TExists, TOuter, bool>> expression) where TExists : class where TOuter : class;
    /// <summary>
    /// Works as opposite of Exists.  Returns a true condition if no rows are returned by the subquery.
    /// <typeparamref name="TExists"/> represents the subquery entity,  and <typeparamref name="TOuter"/> represents the
    /// outer query entity.
    /// </summary>
    /// <typeparam name="TExists">Subquery table entity.</typeparam>
    /// <typeparam name="TOuter">Outer query table.</typeparam>
    ...
    /// include additional filter criteria.</param>
    /// <returns><see cref="IInsert{TInsert, T, T2}"/> object to allow user to chain query methods.</returns>
    IInsert<TInsert, T, T2, T3> WhereNotExists<TExists, TOuter>(Expression<Func<TExists, TOuter, bool>> expression) where TExists : class where TOuter : class;
    /// <summary>
    ...
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> WhereSubquery<TColumn>(Expression<Func<T, T2, T3, TColumn>> expression, Func<ISubquery> subquery);
    /// <summary>
    ...
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> GroupBy<TProperty>(Expression<Func<T, T2, T3, TProperty>> column);
    /// <summary>
    ...
    /// <param name="column">Column.</param>
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> GroupBy(Expression<Func<T, T2, T3, object>> column);
    /// <summary>
    ...
    /// <returns><see cref="IInsert{TInsert, T, T2, T3}"/> object to allow user to chain statement methods.</returns>
    IInsert<TInsert, T, T2, T3> Having(Expression<Func<T, T2, T3, bool>> condition);
}